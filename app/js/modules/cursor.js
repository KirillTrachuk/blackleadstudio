...
export default class Cursor extends Component {
    ...
    _mouseMoveHandler(e) {
        ...
        gsap.to(this._cursorSmall, 0.0, { x: this._clientX - cursorSmallWidth / 2 + borderWidth, y: this._clientY - cursorSmallHeight / 2 + borderWidth, rotation: 0.01 });
        gsap.to(this._cursorLarge, this._followDuration, { x: (this._clientX - cursorLargeWidth / 2), y: (this._clientY - cursorLargeHeight / 2), rotation: 0.01 });
    ...
    _mouseDown() {
        if (!this._cursorOnLink) {
            gsap.fromTo(this._cursorLarge, 0.25, { scale: 1 }, { scale: 0.8 });
        } else {
            gsap.fromTo(this._cursorLarge, 0.25, { scale: 1.5 }, { scale: 1 });
            gsap.fromTo(this._cursorSmall, 0.25, { scale: 1 }, { scale: 0.8 });
        }
    }

    _mouseUp() {
        if (!this._cursorOnLink) {
            gsap.fromTo(this._cursorLarge, 0.25, { scale: 0.8 }, { scale: 1 });
        } else {
            gsap.fromTo(this._cursorLarge, 0.25, { scale: 1 }, { scale: 1.5 });
            gsap.fromTo(this._cursorSmall, 0.25, { scale: 0.8 }, { scale: 1 });
        }
    }
    ...
    _showCursorFirstTime() {
        gsap.to(this._cursorSmall, 0.7, { opacity: 1, scale: 1 });
        gsap.to(this._cursorLarge, 0.7, { opacity: 1, scale: 1 });
    }

    _scaleUp() {
        gsap.to(this._cursorSmall, 0.2, { scale: 0 });
        gsap.to(this._cursorLarge, 0.4, { scale: 1.5 });
    }

    _holdUp() {
        gsap.to(this._cursorSmall.style, 0.2, { background: 'transparent' });
        gsap.to(this._cursorLarge, 0.4, { scale: 1.5 });
        gsap.to(this._cursorHold, 0.3, { scale: 1 });
    }

    _holdDown() {
        gsap.to(this._cursorSmall.style, 0.2, { background: '#FFB4A1' });
        gsap.to(this._cursorLarge, 0.4, { scale: 1 });
        gsap.to(this._cursorHold, 0.1333, { scale: 0 });
    }

    _scaleDown() {
        gsap.to(this._cursorSmall, 0.2, { scale: 1 });
        gsap.to(this._cursorLarge, 0.4, { scale: 1 });
    }

    _showCursor() {
        gsap.to(this._cursorSmall, 0.2, { opacity: 1, scale: 1 });
        gsap.to(this._cursorLarge, 0.2, { opacity: 1, scale: 1 });
        gsap.to(this._cursorHold, 0.2, { opacity: 1 });
    }

    _hideCursor() {
        gsap.to(this._cursorSmall, 0.2, { opacity: 0, scale: 1 });
        gsap.to(this._cursorLarge, 0.2, { opacity: 0, scale: 1 });
        gsap.to(this._cursorHold, 0.2, { opacity: 0 });
    }
}
